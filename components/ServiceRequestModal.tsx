...
const ServiceRequestModal: React.FC<ServiceRequestModalProps> = ({ isOpen, onClose, selectedService, services }) => {
    ...
    // Update displayed service details when dropdown changes
    const currentService = services.find(s => s.id === formData.serviceId) || selectedService;
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl relative animate-in zoom-in-95 duration-200">
                ...
                <div className="p-8">
                    <div className="text-center mb-8">
                        ...
                        {currentService && (
                            <div className="mb-6 p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl flex items-center gap-4 text-left rtl:text-right">
                                <div className="w-16 h-16 rounded-xl overflow-hidden shrink-0">
                                    <img
                                        src={currentService.image_url}
                                        alt={language === 'ar' ? currentService.title_ar : currentService.title_en}
                                        className="w-full h-full object-cover"
                                    />
                                </div>
                                <div>
                                    <h3 className="font-bold text-slate-900 dark:text-white">
                                        {language === 'ar' ? currentService.title_ar : currentService.title_en}
                                    </h3>
                                    <p className="text-sm text-teal-600 font-bold">
                                        {language === 'ar' ? currentService.price_start_ar : currentService.price_start_en} {language === 'en' ? 'KWD' : 'د.ك'}
                                    </p>
                                </div>
                            </div>
                        )}
