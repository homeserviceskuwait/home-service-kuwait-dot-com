...
const Header: React.FC = () => {
  ...
  const content = CONTENT[language].nav;
  ...
  // Using absolute paths with hashes for cross-page navigation capability
  const navLinks = [
    { name: content.home, href: '/' },
    { name: content.services, href: '/#services' },
    { name: content.whyUs, href: '/#features' },
  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-[60] lg:hidden transition-opacity duration-300 ${isMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div 
          className={`absolute top-0 w-[85%] max-w-sm h-full bg-white dark:bg-slate-900 shadow-2xl transition-transform duration-300 ease-out transform ${
            isRTL 
