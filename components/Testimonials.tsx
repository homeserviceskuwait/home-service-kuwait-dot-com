            ...
            <div className="container mx-auto px-4 relative z-10">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 md:mb-16 gap-6">
                    <div className="max-w-xl">
                        <span className="text-teal-600 dark:text-teal-400 font-extrabold tracking-widest uppercase text-xs">
                        ...
                        <h2 className="text-3xl md:text-5xl font-bold text-slate-900 dark:text-white mt-3 leading-tight">
                            {language === 'en' ? 'Trusted by Kuwait' : 'موثوق في الكويت'}
                        </h2>
                    </div>
                ...
                <div className="flex overflow-x-auto snap-x snap-mandatory -mx-4 px-4 pb-6 md:pb-0 md:mx-0 md:px-0 md:grid md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 hide-scrollbar">
                    {loading ? (
                        ...
                        Array.from({ length: 4 }).map((_, index) => (
                            <div key={index} className="min-w-[85vw] md:min-w-0 snap-center group relative bg-slate-50 dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 animate-pulse">
                                {/* Giant Quote Mark */}
                                <div className={`absolute top-6 ${isRTL ? 'left-6' : 'right-6'} text-slate-200 dark:text-slate-700`}>
                                ...
                                <div className="relative z-10 flex flex-col h-full">
                                    <div className="mb-6 flex gap-1">
                                        {[...Array(5)].map((_, i) => (
                                            <div key={i} className="w-3.5 h-3.5 bg-slate-200 dark:bg-slate-700 rounded"></div>
                                        ))}
                                    </div>
                            ...
                            <div key={testimonial.id} className={`min-w-[85vw] md:min-w-0 snap-center group relative bg-slate-50 dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 transition-all duration-300 hover:shadow-xl hover:shadow-slate-200/50 dark:hover:shadow-black/30 hover:-translate-y-1`}>
                                ...
                                <div className="relative z-10 flex flex-col h-full">
                                    ...
                                    <p className="text-slate-700 dark:text-slate-300 text-base leading-relaxed mb-8 flex-1 font-medium">
                                        "{language === 'ar' ? (testimonial.comment_ar || testimonial.comment) : testimonial.comment}"
                                    </p>
                                    ...
                                    <div className="flex items-center gap-4 mt-auto pt-6 border-t border-slate-200 dark:border-slate-800">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-slate-900 dark:text-white text-sm">
                                                {language === 'ar' ? (testimonial.name_ar || testimonial.name) : testimonial.name}
                                            </h4>
                                            <span className="text-xs text-slate-500 dark:text-slate-500 uppercase font-bold tracking-wider">
                                                {language === 'ar' ? (testimonial.role_ar || testimonial.role) : testimonial.role}
                                            </span>
                                        </div>
                                    </div>
                                </div>
